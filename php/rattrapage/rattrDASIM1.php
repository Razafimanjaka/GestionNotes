...
<body>
    ...
    <div class="container-fluid">
        <form method="post">
            <center><h2>SESSION DE RATTRAPAGE : DAII M1 </h2></center><br><br>
            <div class="row">
               <div class="col-md-3">
                    ...
                    <?php 
                        if(isset($_POST['affiche'])){?>
                            <table>
                                ...
                                <?php 
                                $rekety = $conn->prepare("SELECT numero_etu,nom,prenom FROM etudiant,note_et,matiere WHERE noteNormal<10 AND codeM=? AND codeM=codeMat AND numero_etu=matricule AND noteRattr is NULL AND niveau='M1' ");
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select name="num" id="" class="form-control">
                                            <?php 
                                                $rekety = $conn->prepare("SELECT numero_etu FROM etudiant,note_et,matiere WHERE noteNormal<10 AND codeM=? AND codeM=codeMat AND numero_etu=matricule AND noteRattr is NULL AND niveau='M1' ");
                                                $rekety->execute(array($_POST['code']));
                                                while($res=$rekety->fetch()){?>
